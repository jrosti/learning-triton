...
@triton.jit
def acc_denom_and_max(d_acc, max_acc, d, max_cur):
    #pdb.set_trace()
...
@triton.jit
def kernel_fwd(x_ptr, y_ptr, denoms_ptr, max_ptr, ds_ptr_stride, lock_ptr, lock_stride, M, N, stride_xm, stride_xn, num_blocks: tl.constexpr, block_size: tl.constexpr):
    pid_m = tl.program_id(1)
    n = tl.program_id(0)
    
    if n >= N or pid_m >= M:
        return
    row_offset = x_ptr + pid_m * stride_xm
    block_range = tl.arange(0, block_size) * stride_xn
    column_offset = n * stride_xn * block_size + block_range
    mask = column_offset < N * stride_xn
    block_idx = row_offset + column_offset
    row_block = tl.load(block_idx, mask=mask, other=FLT_MIN)
    denom_next, max_chunk = tl.reduce((tl.where(mask, 1.0, 0), row_block), axis=0, combine_fn=acc_denom_and_max)
    tl.atomic_add(denoms_ptr + pid_m * ds_ptr_stride + n * stride_xn, denom_next)
    ...
    while tl.atomic_cas(this_lock_ptr, num_blocks, num_blocks) < num_blocks:
        pass

    ds = tl.load(denoms_ptr + pid_m * ds_ptr_stride + tl.arange(0,num_blocks))
    ms = tl.load(max_ptr + pid_m * ds_ptr_stride + tl.arange(0,num_blocks))
    d_tot, m_tot = tl.reduce((ds, ms), axis=0, combine_fn=acc_denom_and_max)
    column_offset = n * stride_xn * block_size + block_range
    block_idx = row_offset + column_offset 
    mask = column_offset < N * stride_xn
    y = tl.load(block_idx, mask=mask, other=FLT_MIN)
    y = tl.exp(y - m_tot) / d_tot
    tl.store(y_ptr + pid_m * stride_xm + column_offset, y, mask=mask)
